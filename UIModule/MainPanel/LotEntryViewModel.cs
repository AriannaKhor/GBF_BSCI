        ...
        private void Lotoperation(string Command)
        {
            if (Command == "StartLot")
            {
                ...
                else
                {
                    ...
                    {
                        ErrorMsg = GetDialogTableValue("TotalBatchQuantityEmpty");
                    }
            ...
            else if (Command == "EndLot")
            {
                if (Global.AccumulateCurrentBatchQuantity == Global.LotInitialTotalBatchQuantity)
                {
                    ...
                    if (dialogResult == ButtonResult.Yes)
                    {
                        ...
                        m_EventAggregator.GetEvent<MachineOperation>().Publish(new SequenceEvent() { TargetSeqName = SQID.SampleSeq, MachineOpr = MachineOperationType.BeginEndLot });
