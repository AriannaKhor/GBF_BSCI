...
namespace UIModule.StandardViews.Services
{
    public class TowerLight : BindableBase
    {
        ...
        private void BuzzerTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
            lock (sender)
            {
                IO.WriteBit((int)OUT.DO0103_Buzzer, IsBuzzerBlinking);
        ...
        private void LightTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
            ...
            {
                if (IsRedBlinking)
                    IO.WriteBit((int)OUT.DO0100_RedTowerLight, IsLightBlinking);
                if (IsYellowBlinking)
                    IO.WriteBit((int)OUT.DO0101_AmberTowerLight, IsLightBlinking);
                if (IsGreenBlinking)
                    IO.WriteBit((int)OUT.DO0102_GreenTowerLight, IsLightBlinking);
            ...
            lock (this)
            {
                try
                {
                    ...
                    if (currentState.RedState == TowerLightState.ON)
                        IO.WriteBit((int)OUT.DO0100_RedTowerLight, true);
                    ...
                    if (currentState.YellowState == TowerLightState.ON)
                        IO.WriteBit((int)OUT.DO0101_AmberTowerLight, true);
                    ...
                    if (currentState.GreenState == TowerLightState.ON)
                        IO.WriteBit((int)OUT.DO0102_GreenTowerLight, true);
